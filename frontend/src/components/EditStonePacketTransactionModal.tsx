/**
 * ============================================
 * EDIT STONE PACKET TRANSACTION MODAL
 * ============================================
 * Mirror of EditDiamondTransactionModal for synthetic-stone packets.
 * Backend reverses old stock + vendor-credit effects in one $transaction,
 * then re-applies new values. Edits blocked server-side if `stone_packet_payments`
...
interface Props {
  transaction: StonePacketTransaction;
  onClose: () => void;
  onSaved?: () => void;
}
...
const STONE_TYPES = ['CZ', 'KUNDAN', 'POLKI', 'AMERICAN_DIAMOND', 'CRYSTAL', 'GLASS', 'OTHER'];
const QUALITIES = ['AAA', 'AA', 'A', 'B'];
...
export default function EditStonePacketTransactionModal({
  transaction,
  onClose,
  onSaved,
}: Props) {
  ...
  const txnTotal = txn.totalValue ?? ((txn.quantity ?? 0) * (txn.pricePerUnit ?? 0));
  ...
  const [formData, setFormData] = useState({
    stoneType: txn.packet?.stoneType ?? '',
    size: txn.packet?.size ?? '',
    color: txn.packet?.color ?? '',
    quality: txn.packet?.quality ?? '',
    quantity: txn.quantity ?? 0,
    unit: txn.unit ?? txn.packet?.unit ?? 'CARAT',
    pricePerUnit: txn.pricePerUnit ?? 0,
  ...
  const validate = (): FieldErrors => {
    ...
    if (!formData.size) e.size = 'Size is required';
    if (!formData.color) e.color = 'Color is required';
    if (!formData.quality) e.quality = 'Quality is required';
    if (!formData.quantity || formData.quantity <= 0)
      e.quantity = 'Quantity must be greater than 0';
    if (isPurchase && (!formData.pricePerUnit || formData.pricePerUnit <= 0))
      e.pricePerUnit = 'Price per unit is required';
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      return updateStonePacketTransaction(transaction.id, payload);
    },
    onSuccess: () => {
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl my-8">
        <div className="flex items-center justify-between px-6 py-4 border-b border-champagne-200 sticky top-0 bg-white rounded-t-2xl z-10">
          <div>
            <h2 className="text-xl font-bold text-onyx-900">Edit Stone Packet Transaction</h2>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <select
                  ...
                  className={inputClass(errors.quality)}
                >
                  <option value="">— select —</option>
                  {QUALITIES.map((q) => (
            ...
            <div className="rounded-2xl border border-champagne-200 bg-gradient-to-r from-pearl-50 via-white to-champagne-50 p-5 flex items-center justify-between">
              ...
              <div className="text-xs text-onyx-400 text-right">
                <p>{formData.quantity} {formData.unit} × ₹{formData.pricePerUnit}/{formData.unit}</p>
              </div>
            </div>
