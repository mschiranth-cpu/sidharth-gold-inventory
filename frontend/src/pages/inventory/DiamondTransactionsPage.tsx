 ...
 * ============================================
 * Full ledger view of every diamond transaction (PURCHASE, ISSUE,
 * RETURN, TRANSFER, ADJUSTMENT) with filters, settlement, edit, delete,
 * and Excel export. Mirrors MetalTransactionsPage in structure.
 */
...
// Per-shape stat card config — top shapes from the Receive/Add Diamond form.
const SHAPE_CARDS: { code: string; label: string; icon: string; accent: string }[] = [
  { code: 'ROUND', label: 'Round', icon: '⚪', accent: 'from-sky-400 to-sky-600' },
  { code: 'PRINCESS', label: 'Princess', icon: '◆', accent: 'from-violet-400 to-violet-600' },
  { code: 'EMERALD', label: 'Emerald', icon: '▭', accent: 'from-emerald-400 to-emerald-600' },
  { code: 'OVAL', label: 'Oval', icon: '⬭', accent: 'from-rose-400 to-rose-600' },
];
...
export default function DiamondTransactionsPage() {
  ...
  const [settleTxn, setSettleTxn] = useState<DiamondTransaction | null>(null);
  ...
  const { data: txns = [], isLoading, refetch, isFetching } = useQuery({
    ...
    queryFn: () => getAllDiamondTransactions(filters),
  });

  // Apply client-side text search + shape filter on top of server results.
  const visibleTxns = useMemo(() => {
    const q = search.trim().toLowerCase();
    return (txns as DiamondTransaction[]).filter((t) => {
      if (filters.shape && t.diamond?.shape !== filters.shape) return false;
      if (!q) return true;
      const hay = [
        t.referenceNumber,
        t.vendor?.name,
        t.vendor?.uniqueCode,
        t.diamond?.stockNumber,
        t.notes,
        t.createdBy?.name,
      ]
        .filter(Boolean)
  ...
  const totals = useMemo(() => {
    ...
    const byShape = new Map<string, { count: number; carats: number; value: number }>();
    for (const t of visibleTxns) {
      const sh = t.diamond?.shape;
      if (!sh) continue;
      const cur = byShape.get(sh) ?? { count: 0, carats: 0, value: 0 };
      cur.count += 1;
  ...
  const del = useMutation({
    mutationFn: (id: string) => deleteDiamondTransaction(id),
    onSuccess: () => {
  ...
  const handleExport = async () => {
    try {
      const blob = await exportDiamondTransactionsXlsx(filters as any);
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `diamond-transactions-${new Date().toISOString().slice(0, 10)}.xlsx`;
    ...
    <div className="p-6 bg-gradient-to-b from-pearl to-white min-h-screen">
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {SHAPE_CARDS.map((s) => {
            const data = totals.byShape[s.code] ?? { count: 0, carats: 0, value: 0 };
            const isActive = filters.shape === s.code;
            return (
              <button
                key={s.code}
                type="button"
                onClick={() =>
                  setFilters((f) => ({
                    ...f,
                    shape: f.shape === s.code ? undefined : s.code,
                  }))
                }
          ...
          {isLoading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {visibleTxns.map((t: DiamondTransaction) => {
                    const meta =
            ...
            <p className="text-sm text-onyx-600 mb-4">
              ...
              {confirmDelete.transactionType === 'PURCHASE' && (
                <span className="block mt-2 text-accent-ruby">
                  Note: PURCHASE rows can only be removed by deleting the diamond
                  itself (which cascades).
                </span>
              )}
            </p>
