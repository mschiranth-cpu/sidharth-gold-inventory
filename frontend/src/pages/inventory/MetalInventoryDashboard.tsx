...
export default function MetalInventoryDashboard() {
  ...
  const fxQ = useQuery({ queryKey: ['fx-usd-inr'], queryFn: fetchUsdInr, staleTime: 5 * 60_000 });
  const ptQ = useQuery({ queryKey: ['spot-XPT'], queryFn: () => fetchSpot('XPT'), refetchInterval: spotMs });
  const pdQ = useQuery({ queryKey: ['spot-XPD'], queryFn: () => fetchSpot('XPD'), refetchInterval: spotMs });
