    ...
    <div className="p-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-onyx-900 mb-2">Metal Inventory Report</h1>
            <p className="text-onyx-500">Comprehensive metal inventory analysis</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-champagne-100">
            <p className="text-sm text-onyx-500 mb-1">Total Gross Weight</p>
            <p className="text-2xl font-bold text-onyx-900">{totalGrossWeight.toFixed(2)}g</p>
          </div>
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-champagne-100">
            <p className="text-sm text-onyx-500 mb-1">Total Pure Weight</p>
            <p className="text-2xl font-bold text-onyx-900">{totalPureWeight.toFixed(2)}g</p>
          </div>
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-champagne-100">
            <p className="text-sm text-onyx-500 mb-1">Total Purchases</p>
            <p className="text-3xl font-bold text-accent-emerald">{purchases.length}</p>
          </div>
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-champagne-100">
            <p className="text-sm text-onyx-500 mb-1">Total Issues</p>
            <p className="text-3xl font-bold text-champagne-700">{issues.length}</p>
          </div>
        </div>

        {/* Stock by Metal Type and Purity */}
        <div className="bg-white rounded-2xl shadow-lg p-6 border border-champagne-100 mb-6">
          <h2 className="text-xl font-bold text-onyx-900 mb-6">Stock by Metal Type & Purity</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {summary.map((item: any, index: number) => (
              <div
                key={index}
                className="p-4 bg-gradient-to-br from-amber-50 to-amber-50 rounded-xl border border-amber-200"
              >
                <h3 className="font-bold text-onyx-900 mb-2">
                  {item.metalType} ({item.purity}K)
                </h3>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-onyx-500">Gross Weight</span>
                    <span className="font-semibold text-onyx-900">
                      {item.totalGrossWeight.toFixed(2)}g
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-onyx-500">Pure Weight</span>
                    <span className="font-semibold text-onyx-900">
        ...
        {/* Transaction Summary */}
        <div className="bg-white rounded-2xl shadow-lg p-6 border border-champagne-100">
          <h2 className="text-xl font-bold text-onyx-900 mb-6">Transaction Summary</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="p-4 bg-green-50 rounded-xl border border-emerald-200">
              <p className="text-sm text-onyx-500 mb-1">Purchases</p>
              <p className="text-3xl font-bold text-accent-emerald">{purchases.length}</p>
            ...
            <div className="p-4 bg-champagne-50 rounded-xl border border-champagne-200">
              <p className="text-sm text-onyx-500 mb-1">Issues</p>
              <p className="text-3xl font-bold text-champagne-700">{issues.length}</p>
              <p className="text-xs text-onyx-400 mt-2">
                Total: {issues.reduce((sum: number, i: any) => sum + i.grossWeight, 0).toFixed(2)}g
              </p>
            </div>
            <div className="p-4 bg-red-50 rounded-xl border border-accent-ruby/30">
              <p className="text-sm text-onyx-500 mb-1">Wastage</p>
              <p className="text-3xl font-bold text-accent-ruby">{wastage.length}</p>
              <p className="text-xs text-onyx-400 mt-2">
